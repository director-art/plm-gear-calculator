...
const { calculateAdvanceRatio, calculateComparison, calculateConfiguration, calculateRatio, classifySlip } = await import('./script.js');
...
test('calculates advance ratio J using speed, shaft RPM and diameter', () => {
  const j = calculateAdvanceRatio(45, 6000 / 1.75, 13.5);
  assert.equal(j.toFixed(2), '0.64');
  assert.equal(calculateAdvanceRatio(45, 0, 13.5), null);
});

test('compares a gearbox replacement', () => {
  const result = calculateComparison({
    oldEngineRpm: 6000, newEngineRpm: 6000, oldRatio: 1.75, newRatio: 27 / 13,
    oldPitch: 13, newPitch: 13, oldDiameter: 13.5, newDiameter: 13.5, actualSpeed: 45,
  });
  ...
  assert.equal(result.oldAdvanceRatio.toFixed(2), '0.64');
  assert.equal(result.newAdvanceRatio.toFixed(2), '0.64');
...
test('compares a propeller replacement and changed engine RPM', () => {
  const result = calculateComparison({
    ...
    oldPitch: 13, newPitch: 15, oldDiameter: 13.5, newDiameter: 14, actualSpeed: 45,
  });
  ...
  assert.ok(result.newAdvanceRatio > result.oldAdvanceRatio);
});
...
test('marks slip invalid when actual speed exceeds theoretical speed', () => {
  const result = calculateComparison({
    ...
    oldPitch: 13, newPitch: 13, oldDiameter: 13.5, newDiameter: 13.5, actualSpeed: 80,
  });
  assert.equal(result.validSlip, false);
  assert.equal(result.newCalculatedSpeed, null);
  assert.equal(result.newAdvanceRatio, null);
...
test('accepts zero actual speed without producing an invalid percentage', () => {
  const result = calculateComparison({
    oldEngineRpm: 6000, newEngineRpm: 6000, oldRatio: 1.75, newRatio: 2.08,
    oldPitch: 13, newPitch: 13, oldDiameter: 13.5, newDiameter: 13.5, actualSpeed: 0,
  });
  assert.equal(result.slip, 100);
  assert.equal(result.newCalculatedSpeed, 0);
  assert.equal(result.oldAdvanceRatio, 0);
  assert.equal(result.newAdvanceRatio, 0);
  assert.equal(result.calculatedChange, null);
});
